...
OUTPUT_ROOT = "./data/qlear_test_by_backend_family"
...
def build_strict_depth_cut_subcircuits(transpiled_circuit: QuantumCircuit):
    ...
    if total_depth == 0:
        return {"Q1": None, "Q2": None, "Q3": None, "depth": 0, "bounds": (0, 0, 0, 0)}
    ...
    return {"Q1": q1, "Q2": q2, "Q3": q3, "depth": total_depth, "bounds": (0, b1, b2, b3)}
...
def compute_dpe_features_strict(raw_circuit: QuantumCircuit, noisy_backend, shots=1024, seed=1997):
    ...
    transpiled_full = transpile(raw_circuit, backend=noisy_backend, optimization_level=0, seed_transpiler=seed)
...
def build_rows_for_distribution(circuit_name, sample_id, ideal_probs, noisy_probs, circuit_feature_dict, dpe_feature_dict, include_noisy_only_states=True):
    rows = []
...
def prepare_test_families():
    _, cut_families = train_test_split(get_all_circuits(), train_size=TRAIN_SPLIT, random_state=RANDOM_STATE)
    rules = read_configuration("../QOIN/Configuration.txt")

    data_circuit_pairs = []
    for family_name in cut_families:
        circuit_obj = get_circuit_class_object(family_name)
        print(f"Preparing test family: {family_name}, ID:{circuit_obj.key_aurguments['ID']}")
        ...
        data_circuit_pairs.append((family_name, data, circuit_obj))
    return data_circuit_pairs


def iter_test_inputs(circuit_obj, data):
    input_type = circuit_obj.key_aurguments["input_type"]
    if input_type == 1:
        for iteration, inp in enumerate(data):
            yield iteration, inp
    elif input_type == 2:
...
def main():
    ...
    test_families = prepare_test_families()

    for bk, _ in BACKENDS:
        backend_dir = os.path.join(OUTPUT_ROOT, bk)
        os.makedirs(backend_dir, exist_ok=True)

        print(f"\nGenerating Q-LEAR test data for backend: {bk}")
        print("-" * 60)

        for family_name, data, circuit_obj in test_families:
            print(f"Processing test family: {family_name}")
            family_rows = []

            for iteration, inp in tqdm(list(iter_test_inputs(circuit_obj, data)), desc=f"{bk}:{family_name}"):
                sample_id = f"{circuit_obj.key_aurguments['ID']}_{iteration}"

                try:
                    ideal_result = circuit_obj.get_result(ideal_backend, inp, number_of_runs=SHOTS, seed=SEED)
                    noisy_result = circuit_obj.get_result(noisy_backends[bk], inp, number_of_runs=SHOTS, seed=SEED)
                ...
                try:
                    dpe_features, transpiled_full = compute_dpe_features_strict(
                        raw_circuit, noisy_backends[bk], shots=SHOTS, seed=SEED
                    )
