...
class StringComparator:
    ...
    def run(self, quantum_instance=None):
        ...
        # Guard against zero useful shots on noisy backends
        if useful_shots_count == 0:
            p_values = [0.0 for _ in self.string_db]
            ham_distances = [self.input_size // self.symbol_length for _ in self.string_db]
            self.results = {
                'p_values': p_values,
                'hamming_distances': ham_distances,
                'prob_of_measuring_register_c_as_0': 0.0,
        ...
        for my_str in self.string_db:
            p_values.append(count_dic.get(my_str, 0) / useful_shots_count)

        probability_of_measuring_register_c_as_0 = float(sum(p_values))

        # Re-normalize p-values so that they sum to 1.0
        if probability_of_measuring_register_c_as_0 != 0:
            for ind in range(len(p_values)):
                p_values[ind] = p_values[ind] / probability_of_measuring_register_c_as_0

        ham_distances = self._convert_p_value_to_hamming_distance(
            p_values, probability_of_measuring_register_c_as_0
        )
...
def _scalable_bitstrings(length, limit=8, include_zero=False):
    values = []
    if include_zero:
        values.append("0" * length)
    if length >= 1:
        values.append("1" * length)
        values.append("1" + "0" * (length - 1))
        values.append("0" * (length - 1) + "1")
    if length >= 2:
        values.append(("10" * ((length + 1) // 2))[:length])
        values.append(("01" * ((length + 1) // 2))[:length])
    if length >= 3:
        mid = ["0"] * length
        mid[length // 2] = "1"
        values.append("".join(mid))
        values.append("1" * (length // 2) + "0" * (length - length // 2))
        values.append("0" * (length // 2) + "1" * (length - length // 2))
...
def _scalable_addition_pairs(bit_lengths, limit_per_length=4):
    pairs = []
...
class addition(abstract_interface):
    ...
    def get_inputs(self):
        return _scalable_addition_pairs(bit_lengths=[3, 4, 5, 6], limit_per_length=3)

    def get_full_inputs(self):
        return _scalable_addition_pairs(bit_lengths=[3, 4, 5, 6], limit_per_length=5)
...
class phase(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        total_qubits = len(input_data)
        counting_qubits = total_qubits - 1
        if counting_qubits < 1:
            raise ValueError("phase circuit requires at least 2 qubits")

        q = QuantumRegister(total_qubits, 'q')
        c = ClassicalRegister(counting_qubits, 'c')
        ...
        for i in range(counting_qubits):
            repeats = max(1, 2 ** min(i, 5))
            for _ in range(repeats):
                circuit.cp(angle, q[i], q[target])
            if input_data[i] == "1":
                circuit.p(pi / (2 ** ((i % 4) + 1)), q[i])
        ...
        circuit = circuit.compose(iqft, qubits=list(range(counting_qubits)))
    ...
    def get_inputs(self):
        ...
        for length in [8, 10, 12, 14]:
            inputs.extend(_scalable_bitstrings(length, limit=3, include_zero=False))
        return inputs

    def get_full_inputs(self):
        inputs = []
        for length in [8, 10, 12, 14]:
            inputs.extend(_scalable_bitstrings(length, limit=6, include_zero=True))
        return inputs
...
class qft(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        circuit = circuit.compose(QFT(num_qubits=n, approximation_degree=0, do_swaps=True, inverse=False, insert_barriers=True, name='qft'))
        circuit = circuit.compose(QFT(num_qubits=n, approximation_degree=0, do_swaps=True, inverse=True, insert_barriers=True, name='iqft'))
        circuit.measure(q, c)
...
class simon(abstract_interface):
    ...
    def get_inputs(self):
        ...
        for length in [5, 6, 7, 8, 9, 10]:
            inputs.extend([x for x in _scalable_bitstrings(length, limit=3, include_zero=False) if "1" in x])
        return inputs

    def get_full_inputs(self):
        inputs = []
        for length in [5, 6, 7, 8, 9, 10]:
            inputs.extend([x for x in _scalable_bitstrings(length, limit=6, include_zero=True) if "1" in x])
        return inputs
...
class ghz(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997):
        ...
        for i in range(input_data - 1):
            circ.cx(i, i + 1)
        circ.measure(range(input_data), range(input_data))
        decomposed = circ.decompose()
        if self.key_aurguments["circuit"]:
            self.key_aurguments["circuit"] = decomposed
        result = quantum_instance.execute(decomposed)
        counts = result.get_counts()
...
class similarity(abstract_interface):
    ...
    def get_result(self, backend, input_data, number_of_runs=1024, seed=1997, use_gpu=False):
        ...
        if use_gpu:
            if isinstance(backend, AerSimulator):
                exec_backend = backend
                exec_backend.set_options(device="GPU", seed_simulator=seed)
            else:
                exec_backend = AerSimulator.from_backend(backend)
                exec_backend.set_options(device="GPU", seed_simulator=seed)

        if isinstance(exec_backend, AerSimulator):
            _backend_name = getattr(exec_backend, "name", None)
            if isinstance(_backend_name, str):
                exec_backend.name = _CallableStr(_backend_name)

        quantum_instance = QuantumInstance(
            exec_backend,
            shots=number_of_runs,
    ...
    def get_inputs(self):
        ...
        pairs = []
        for length in [4, 6, 8]:
            strings = _scalable_bitstrings(length, limit=4, include_zero=True)
            for a, b in product(strings[:2], strings[2:4]):
                pairs.append((a, b))
        return pairs
    ...
    def get_full_inputs(self):
        ...
        for length in [4, 5, 6, 7, 8]:
            strings = _scalable_bitstrings(length, limit=6, include_zero=True)
            for a, b in product(strings[:3], strings[3:6]):
                pairs.append((a, b))
        return pairs
