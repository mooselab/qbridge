...
def circ_to_dag_with_data(circ, device_name, n_qubit=10):
    ...
    def _build_qubit_calib_arrays(noise_dict, n_qubit):
        ...
        for q in range(n_qubit):
            ...
            if "T1" in qd: T1[q] = float(qd["T1"])                 # unit: μs
            if "T2" in qd: T2[q] = float(qd["T2"])                 # unit: μs
    ...
    D_NODE = NUM_NODE_TYPE + n_qubit + 14
    ...
    for topo_idx, node in enumerate(order):
        ...
        # Qubit multi-hot (by physical index)
        for q in qubit_idxs:
            if 0 <= q < n_qubit:
                data[NUM_NODE_TYPE + q] = 1.0

        base = NUM_NODE_TYPE + n_qubit
