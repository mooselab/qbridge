...
class _GraphSample:
    """x/edge_index/edge_attr/backend_id"""
    def __init__(self, x, edge_index, edge_attr, backend_id: int):
        self.x = x
...
class trainer:
    def __init__(self, model, device, criterion, optimizer, scheduler, loaders,
                backend, mode, seed, cut=None, extra_data=None, table_by_cirid=None, xobs_std=None ,default_shots=1024):
        self.model = model
        self.device = device
    ...
    def _forward_with_cached_h(self, batch, train_encoder=True):
        ...
        # First extract each graph's backend_id and embed it (model already has self.backend_embed)
        backend_ids = torch.tensor(
            [int(d.backend_id) for d in data_list],
            dtype=torch.long, device=self.device
        )  # [B_graph]
        backend_vecs = self.model.embed_backends(backend_ids)  # [B_graph, D_b]
        ...
        for h, cir_id, d, bvec in zip(h_list, cir_ids, data_list, backend_vecs):
            ...
            for row in rows:
                xo, y = _make_x_obs_from_row(row, shots=shots)     # xo: [4]
                xo_aug = torch.cat([xo.to(self.device), bvec], dim=-1)  # [4 + D_b]
                h_all.append(h)
                x_obs_all.append(xo_aug)
                y_all.append(y.to(self.device))
                cir_ids_exp.append(cir_id)

        h_all     = torch.stack(h_all, dim=0)        # [B_total, H]
        x_obs_all = torch.stack(x_obs_all, dim=0)    # [B_total, 4 + D_b]
        # Assertion: aligned with the Linear input of the head
        assert x_obs_all.size(1) == self.model.head.film.in_features, \
            f"x_obs dim {x_obs_all.size(1)} != head expects {self.model.head.film.in_features}"
    ...
    def train(self):
        ...
        accum_steps = getattr(configs, "accum_steps", 1)
        ...
        # model_path = f"../model/{self.mode}/seed_{str(self.seed)}/full_model/"
        model_path = f"../larger_circuits/model/{self.mode}/seed_{str(self.seed)}/full_model/"
        if self.cut:
            model_path += self.cut + "/"
    ...
    def saveall(self):
        ...
        # RQ2
        # all_path = f"../model/evaluation_data/seed_{str(self.seed)}/full_model/"
        all_path = f"../larger_circuits/model/evaluation_data/seed_{str(self.seed)}/full_model/"
        # RQ3
        # all_path = f"../model/testing_data/seed_{str(self.seed)}/full_model/"
