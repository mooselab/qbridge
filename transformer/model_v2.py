...
class QIONFiLMModel(nn.Module):
    ...
    def __init__(
        ...
        # === Added: backend embedding ===
        num_backends = _get(args, "num_backends", 23)            # number of backends
        backend_embed_dim = _get(args, "backend_embed_dim", 16)  # embedding dimension（adjustable）
        self.backend_embed = nn.Embedding(num_backends, backend_embed_dim)
        ...
        # === Adjust the input dimension of head: obs_dim + backend_embed_dim ===
        obs_dim_final = obs_dim + backend_embed_dim
        self.head = FiLMNoiseHead(h_dim=embed_dim, obs_dim=obs_dim_final, hidden=head_hidden, direct_branch=True)

    # Expose a convenient function to retrieve embedding vectors
    def embed_backends(self, backend_ids: torch.Tensor) -> torch.Tensor:
        """
        backend_ids: [B_graph] LongTensor, [0, num_backends-1]
        return: [B_graph, backend_embed_dim]
        """
        return self.backend_embed(backend_ids.long())
