...
class trainer:
    ...
    def train(self):
        ...
        # model_path = f"../model/{self.mode}/seed_{str(self.seed)}/{self.backend}/"
        model_path = f"../larger_circuits/model/{self.mode}/seed_{str(self.seed)}/{self.backend}/"
        if self.cut:
            model_path += self.cut + "/"
    ...
    def saveall(self):
        ...
        # RQ1
        # all_path = f"../model/evaluation_data/seed_{str(self.seed)}/{self.backend}/"
        all_path = f"../larger_circuits/model/evaluation_data/seed_{str(self.seed)}/{self.backend}/"
        # RQ3
        # all_path = f"../model/testing_data/seed_{str(self.seed)}/{self.backend}/"
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch in self.loaders["test"]:
                ...
                #     else:
                #         d["ideal"] = [0.0] * len(d["ideal"])
                #         d["pred"] = [0.0] * len(d["pred"])
