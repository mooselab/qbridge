...
T_RUNS = 10
SHOTS = 1024
ITERATIONS_PER_FAMILY = 100

OUTPUT_ROOT = "./data/qraft_tuning_data"
os.makedirs(OUTPUT_ROOT, exist_ok=True)

# Same backend list as your current scripts
backends = [
...
# Same split logic as your DataGeneration / BaselineTuner
...
def append_family_columns(df, family_name, family_id):
    df = df.copy()
    df["Family"] = family_name
    df["FamilyID"] = family_id
    return df


for backend_idx, (bk, qubit_size) in enumerate(backends):
    print(f"\n==============================")
    print(f"Generating Qraft tuning data for backend: {bk}")
    print(f"==============================")
    ...
    for cut in CUTs:
        ...
        for iteration in tqdm(range(ITERATIONS_PER_FAMILY), desc=f"{bk}-{family_name}", leave=False):
            inp = test_inputs[iteration % len(test_inputs)]
            circuit = copy.deepcopy(circuit_template)

            try:
                # Instantiate/update the circuit object on the ideal executor
        ...
        if len(full_rows) == 0:
            print(f"[WARN] no rows generated for backend={bk}, family={family_name}")
            continue

        df_full = pd.DataFrame(full_rows)
        df_model = pd.DataFrame(model_rows)

        # Save per backend × family
...
print("\nAll family-specific Qraft tuning data generated.")